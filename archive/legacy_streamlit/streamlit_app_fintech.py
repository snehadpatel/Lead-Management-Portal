...
API_BASE_URL = os.environ.get("LUME_API_BASE_URL", "http://localhost:8000")
...
def fetch_api_json(path: str) -> Dict[str, Any]:
    """Fetch JSON from the FastAPI backend with short caching."""
    try:
...
def fetch_live_alerts() -> Dict[str, Any]:
    return fetch_api_json("/alerts/live")


@st.cache_data(ttl=15)
def fetch_market_impact() -> Dict[str, Any]:
    return fetch_api_json("/market/impact/latest")
...
def render_live_alerts():
    """Render live alerts and the latest market impact snapshot."""
    st.markdown("### 🚨 Live Alerts")
    st.caption("Auto-refreshes every 30 seconds while this tab is open.")
    st_autorefresh(interval=30000, key="lume_live_alerts_refresh")

    alerts_data = fetch_live_alerts()
    impact_data = fetch_market_impact()

    col1, col2 = st.columns([2, 1])

    with col1:
        if alerts_data and alerts_data.get("alerts"):
            severity = str(alerts_data.get("severity", "moderate")).upper()
            color = {
                "CRITICAL": "#ef4444",
                "HIGH": "#f97316",
                "POSITIVE": "#10b981",
            ...
            st.markdown(f"<div class='fintech-card' style='border-left: 4px solid {color};'>", unsafe_allow_html=True)
            st.markdown(f"**Severity:** <span style='color:{color};'>{severity}</span>", unsafe_allow_html=True)
            st.markdown(f"**Confidence:** {alerts_data.get('confidence', 0):.2f}")
            for item in alerts_data.get("alerts", [])[:10]:
                st.markdown(f"- {item}")
            st.markdown("</div>", unsafe_allow_html=True)
        else:
    ...
    with col2:
        st.markdown('<div class="fintech-card">', unsafe_allow_html=True)
        st.markdown("#### Market Impact Snapshot")
        if impact_data:
            st.metric("Impact Severity", str(impact_data.get("severity", "N/A")).upper())
            st.metric("Confidence", f"{impact_data.get('confidence', 0):.2f}")
            impacted = impact_data.get("affected_sectors", [])
            st.write("**Affected sectors:**")
            st.write(", ".join(impacted) if impacted else "None")
            st.write("**Top Actions:**")
            for action in impact_data.get("recommended_actions", [])[:3]:
                st.write(f"- {action}")
        else:
            st.info("Market impact not available.")
        st.markdown('</div>', unsafe_allow_html=True)
...
def main():
    ...
    # Main content tabs
    tab_live, tab_alerts, tab_leads, tab_investors, tab_market, tab_bi = st.tabs([
        "⚡ Live Pulse",
        "🚨 Alerts",
        "🎯 Lead Intelligence", 
        "👥 Investor Analytics", 
